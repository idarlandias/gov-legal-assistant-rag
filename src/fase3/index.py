...
from src.fase3.corpus import CORPUS_DIR, Dispositivo, load_manifest, save_manifest, segment, to_jsonl
...
def index(chunks: list[Chunk], collection, batch: int = 200, progress=print) -> int:
    for start in range(0, len(chunks), batch):
        ...
        progress(f"  indexados {min(start + batch, len(chunks))}/{len(chunks)}")
...
def rebuild(fontes: list[Fonte] = FONTES, progress=print) -> dict[str, dict]:
    ...
    pipe = RAGPipeline(collection_name=COLLECTION)  # mesma config de embedding do app
    try:
        pipe.chroma_client.delete_collection(COLLECTION)
    except Exception:
        pass  # coleção ainda não existia
    collection = pipe.chroma_client.get_or_create_collection(
        name=COLLECTION, embedding_function=pipe.embed_fn
    )
    total = index(chunks, collection, progress=progress)
    if total != len(chunks):
        raise RuntimeError(f"Chroma tem {total} chunks, esperado {len(chunks)}")

    manifest = load_manifest()
    for fid, st in stats.items():
        manifest.setdefault(fid, {})["indice"] = {**st, "embed_model": pipe.embed_model}
